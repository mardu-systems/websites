...
export function IntegrationCard({ item }: { item: IntegrationListItemDto }) {
  return (
    <article className="group border border-black/12 bg-white/45 p-4 transition-colors hover:bg-white/65">
      <Link href={`/integrations/${item.slug}`} className="block">
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1">
            <h3 className="text-2xl leading-[1.06] tracking-[-0.02em] text-foreground">{item.title}</h3>
            <p className="mt-2 line-clamp-3 text-sm text-foreground/75">{item.shortDescription}</p>
          </div>
        </div>

        <div className="mt-4 flex items-center gap-2">
          <IntegrationStatusBadge status={item.status} />
          {item.vendor ? <span className="text-xs text-foreground/55">{item.vendor}</span> : null}
        </div>

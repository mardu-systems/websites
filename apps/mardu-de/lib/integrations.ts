...
export const getFeaturedIntegrations = async (limit = 8): Promise<IntegrationListItemDto[]> => {
  ...
  return applySort(all.filter((item) => item.featured), 'featured')
    .slice(0, Math.max(1, limit))
    .map(withIntegrationLogo);
};
...
export const getIntegrations = async (
  ...
  return {
    items: filtered.slice(start, start + normalized.limit).map(withIntegrationLogo),
    page,
...
export const getIntegrationBySlug = async (slug: string): Promise<IntegrationDetailDto | null> => {
  try {
    ...
    return detail ? withIntegrationLogo(detail) : detail;
  } catch {
    return null;
  }
};
...
export const getRelatedIntegrations = async (
  ...
  return applySort(
    ...
    'featured',
  ).slice(0, Math.max(1, limit)).map(withIntegrationLogo);
};

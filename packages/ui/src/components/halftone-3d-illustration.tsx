// @ts-nocheck -- The public DTO/Props API below is typed; the imported Three.js
// shader prototype keeps dynamic material/uniform fields that are validated via
// the consuming app checks until the renderer internals are split further.
'use client';
...
const DEFAULT_PREVIEW_DISTANCE = 6.1;
...
function resolveHalftone3DSettings(
...
function resolveHalftone3DPose(
...
function createInteractionState(initialPose: Halftone3DPose) {
  return {
...
function createGeometry(shapeKey: Halftone3DShapeKey) {
...
type HalftoneCanvasMountOptions = {
...
async function mountHalftoneCanvas(
  options: HalftoneCanvasMountOptions,
): Promise<HalftoneCanvasController | null> {
  const {
    container,
    initialPose,
    onError,
  ...
  let currentShapeKey = options.shapeKey;
  let baseCameraDistance = options.previewDistance;
  ...
  canvas.style.cursor = currentSettings.animation.followDragEnabled ? 'grab' : 'default';
  ...
  const primaryLight = new THREE.DirectionalLight(0xffffff, currentSettings.lighting.intensity);
  setPrimaryLightPosition(
    primaryLight,
    currentSettings.lighting.angleDegrees,
    currentSettings.lighting.height,
  );
  scene3d.add(primaryLight);

  const fillLight = new THREE.DirectionalLight(
    0xffffff,
    currentSettings.lighting.fillIntensity,
  );
  ...
  const ambientLight = new THREE.AmbientLight(
    0xffffff,
    currentSettings.lighting.ambientIntensity,
  );
  scene3d.add(ambientLight);

  const material = createHalftoneMaterial();
  applyHalftoneMaterialSettings(material, currentSettings.material, materialAssets);
  ...
  const halftoneMaterial = new THREE.ShaderMaterial({
    ...
    uniforms: {
      ...
      tile: { value: currentSettings.halftone.scale },
      s_3: { value: currentSettings.halftone.power },
      s_4: { value: currentSettings.halftone.width },
      applyToDarkAreas: {
        value: currentSettings.halftone.toneTarget === 'dark' ? 1 : 0,
      },
      dashColor: { value: new THREE.Color(currentSettings.halftone.dashColor) },
      hoverDashColor: {
        value: new THREE.Color(currentSettings.halftone.hoverDashColor),
      },
      time: { value: 0 },
      waveAmount: { value: 0 },
      waveSpeed: { value: 1 },
  ...
  const applyRuntimeSettings = (nextSettings: Halftone3DSettings) => {
    currentSettings = nextSettings;
    primaryLight.intensity = currentSettings.lighting.intensity;
    fillLight.intensity = currentSettings.lighting.fillIntensity;
    ambientLight.intensity = currentSettings.lighting.ambientIntensity;
    ...
    halftoneMaterial.uniforms.dragFlowStrength.value = currentSettings.animation
      .dragFlowEnabled
      ? currentSettings.animation.dragFlowStrength
      : 0;
    canvas.style.cursor = currentSettings.animation.followDragEnabled
  ...
  const handleWindowPointerMove = (event) => {
    ...
    if (!interaction.dragging || !currentSettings.animation.followDragEnabled) {
      return;
    }

    const deltaX = (event.clientX - interaction.pointerX) * currentSettings.animation.dragSens;
    const deltaY = (event.clientY - interaction.pointerY) * currentSettings.animation.dragSens;
  ...
  const handlePointerUp = () => {
    ...
    const springImpulse = Math.max(currentSettings.animation.springStrength * 10, 1.2);
  ...
  const renderFrame = (timestamp) => {
    ...
    if (currentSettings.animation.autoRotateEnabled) {
      ...
      baseRotationY += interaction.autoElapsed * currentSettings.animation.autoSpeed;
      baseRotationX += Math.sin(interaction.autoElapsed * 0.2) * currentSettings.animation.autoWobble;
    }

    if (currentSettings.animation.floatEnabled) {
      const floatPhase = elapsedTime * currentSettings.animation.floatSpeed;
      const driftAmount = (currentSettings.animation.driftAmount * Math.PI) / 180;

      meshOffsetY += Math.sin(floatPhase) * currentSettings.animation.floatAmplitude;
    ...
    if (currentSettings.animation.breatheEnabled) {
      meshScale *=
        1 +
        Math.sin(elapsedTime * currentSettings.animation.breatheSpeed) *
          currentSettings.animation.breatheAmount;
    }
    ...
    if (currentSettings.animation.rotateEnabled) {
      ...
      const rotateProgress = currentSettings.animation.rotatePingPong
        ? Math.sin(interaction.rotateElapsed * currentSettings.animation.rotateSpeed) * Math.PI
        : interaction.rotateElapsed * currentSettings.animation.rotateSpeed;

      if (currentSettings.animation.rotatePreset === 'axis') {
        const axisDirection =
          currentSettings.animation.rotateAxis.startsWith('-') ? -1 : 1;
        const axisProgress = rotateProgress * axisDirection;

        if (
          currentSettings.animation.rotateAxis === 'x' ||
          currentSettings.animation.rotateAxis === 'xy' ||
          currentSettings.animation.rotateAxis === '-x' ||
          currentSettings.animation.rotateAxis === '-xy'
        ) {
        ...
        if (
          currentSettings.animation.rotateAxis === 'y' ||
          currentSettings.animation.rotateAxis === 'xy' ||
          currentSettings.animation.rotateAxis === '-y' ||
          currentSettings.animation.rotateAxis === '-xy'
        ) {
        ...
        if (
          currentSettings.animation.rotateAxis === 'z' ||
          currentSettings.animation.rotateAxis === '-z'
        ) {
          baseRotationZ += axisProgress;
        }
      } else if (currentSettings.animation.rotatePreset === 'lissajous') {
      ...
      } else if (currentSettings.animation.rotatePreset === 'orbit') {
      ...
      } else if (currentSettings.animation.rotatePreset === 'tumble') {
    ...
    if (currentSettings.animation.lightSweepEnabled) {
      const lightPhase = elapsedTime * currentSettings.animation.lightSweepSpeed;
      lightAngle += Math.sin(lightPhase) * currentSettings.animation.lightSweepRange;
      lightHeight +=
    ...
    if (currentSettings.animation.followHoverEnabled) {
      const rangeRadians = (currentSettings.animation.hoverRange * Math.PI) / 180;

      if (
        currentSettings.animation.hoverReturn ||
        interaction.mouseX !== 0.5 ||
        interaction.mouseY !== 0.5
      ) {
      ...
      easing = currentSettings.animation.hoverEase;
    }
    ...
    if (currentSettings.animation.autoRotateEnabled && !currentSettings.animation.followHoverEnabled && !currentSettings.animation.followDragEnabled) {
    ...
    if (currentSettings.animation.cameraParallaxEnabled) {
      const cameraRange = currentSettings.animation.cameraParallaxAmount;
      const cameraEase = currentSettings.animation.cameraParallaxEase;
    ...
    if (!currentSettings.halftone.enabled) {
  ...
  return {
    ...
    update: (nextOptions) => {
      ...
      baseCameraDistance = nextOptions.previewDistance;
      applyRuntimeSettings(nextOptions.settings);
...
export function Halftone3DIllustration({
  ...
  const resolvedSettings = resolveHalftone3DSettings(shapeKey, settings);

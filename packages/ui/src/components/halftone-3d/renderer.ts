...
type ContainedImageRectOptions = {
  imageHeight: number;
  imageWidth: number;
...
type ImageFootprintOptions = Omit<ContainedImageRectOptions, "zoom"> & {
...
type HalftoneTransmissionUniforms = {
  ...
  refractionEnvMap: Uniform<THREE.Texture | null>;
  ...
  useEnvMapRefraction: Uniform<number>;
};
...
function createBox3Corners(bounds: THREE.Box3): THREE.Vector3[] {
...
function getContainedImageRect({
  ...
  viewportWidth,
  zoom,
}: ContainedImageRectOptions): ViewportRect | null {
  if (
    imageWidth <= 0 ||
    imageHeight <= 0 ||
    viewportWidth <= 0 ||
...
class HalftoneTransmissionMaterial extends THREE.MeshPhysicalMaterial {
  ...
  declare halftoneUniforms: HalftoneTransmissionUniforms;
  ...
  constructor(samples = 10) {
    ...
    (
      Object.keys(this.halftoneUniforms) as Array<
        keyof HalftoneTransmissionUniforms
      >
    ).forEach((key) => {
      Object.defineProperty(this, key, {
        ...
        set: (value: HalftoneTransmissionUniforms[typeof key]["value"]) => {
...
function disposeEnvironmentScene(scene: THREE.Scene): void {
...
function createSolidEnvironmentTexture(
  renderer: THREE.WebGLRenderer,
): THREE.Texture {
...
function getTextureImageSize(texture: THREE.Texture): {
  ...
  const image = texture.image as TextureImageSizeSource | undefined;
...
function createFallbackGlassBackdropTexture(
  renderer: THREE.WebGLRenderer,
): THREE.Texture {
...
async function loadGlassEnvironmentTexture(
  renderer: THREE.WebGLRenderer,
): Promise<{
...
function disposeHalftoneMaterialAssets(
  materialAssets: HalftoneMaterialAssets,
): void {
...
export async function mountHalftoneCanvas(
  ...
  const { container, initialPose, onError, signal } = options;
  ...
  const primaryLight = new THREE.DirectionalLight(
    0xffffff,
    currentSettings.lighting.intensity,
  );
  ...
  const renderFrame = (timestamp = performance.now()) => {
    ...
    if (currentSettings.animation.floatEnabled) {
      ...
      const driftAmount =
        (currentSettings.animation.driftAmount * Math.PI) / 180;
    ...
    if (currentSettings.animation.rotateEnabled) {
      ...
      if (currentSettings.animation.rotatePreset === "axis") {
        const axisDirection = currentSettings.animation.rotateAxis.startsWith(
          "-",
        )
          ? -1
          : 1;
    ...
    if (currentSettings.animation.lightSweepEnabled) {
      const lightPhase =
        elapsedTime * currentSettings.animation.lightSweepSpeed;
      lightAngle +=
        Math.sin(lightPhase) * currentSettings.animation.lightSweepRange;
      lightHeight +=
    ...
    if (currentSettings.animation.followHoverEnabled) {
      const rangeRadians =
        (currentSettings.animation.hoverRange * Math.PI) / 180;
    ...
    if (
      currentSettings.animation.autoRotateEnabled &&
      !currentSettings.animation.followHoverEnabled &&
      !currentSettings.animation.followDragEnabled
    ) {
  ...
  return {
    ...
    update: (nextOptions) => {
      ...
      if (baseCameraDistance !== nextOptions.previewDistance) {
        baseCameraDistance = nextOptions.previewDistance;
      }

      if (currentSettings !== nextOptions.settings) {

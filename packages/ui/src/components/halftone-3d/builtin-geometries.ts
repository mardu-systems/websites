...
import type { Halftone3DShapeKey } from "./types";

type ReliefGeometryOptions = {
...
function makeArrowTarget(): THREE.BufferGeometry {
  const targetParts = [];
  const arrowParts = [];
...
function makeDollarCoin(): THREE.BufferGeometry {
  const parts = [];

...
import type {
  EditorialShapeAnchor,
  EditorialShapeConfig,
  EditorialShapeFade,
  EditorialShapeSilhouette,
...
const shapeAnchorClasses: Record<EditorialShapeAnchor, string> = {
  ...
  "top-center":
    "left-1/2 top-5 -translate-x-1/2 items-center justify-start md:top-7",
...
function isShapeCellVisible(
  ...
  switch (silhouette) {
    case "shield": {
      if (rowIndex <= rows * 0.14) return distanceFromCenter <= cols * 0.28;
    ...
    case "flow": {
      ...
      const arrowHead =
        colIndex >= cols * 0.56 &&
        Math.abs(rowIndex - rows * 0.5) <=
          Math.max(0.9, (cols - colIndex) * 0.38);
      const inletBars =
        colIndex <= cols * 0.14 &&
        rowIndex >= rows * 0.22 &&
        rowIndex <= rows * 0.78 &&
    ...
    case "check": {
      const leftLeg =
        Math.abs(rowIndex - (rows * 0.56 + colIndex * 0.9)) <= 1.15;
      const rightLeg =
        Math.abs(rowIndex - (rows * 0.9 - colIndex * 0.72)) <= 1.15 &&
        colIndex >= cols * 0.34;
...
export function EditorialShapePattern({
  shape,
}: {
